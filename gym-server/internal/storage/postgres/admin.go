...
func (s *Storage) UpdateAdmin(ctx context.Context, updateFields map[string]interface{}) (*models.Admin, error) {
	const op = "postgres.UpdateAdmin"

	admin_id := ctx.Value(ctxkey.UserKey)

	query, args, _ := goqu.Update("admins").
		Set(updateFields).
...
func readRow(row *sql.Row) (*models.Admin, error) {
